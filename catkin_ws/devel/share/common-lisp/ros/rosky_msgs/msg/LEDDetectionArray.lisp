...
(cl:defclass <LEDDetectionArray> (roslisp-msg-protocol:ros-message)
  ((detections
    :reader detections
    :initarg :detections
    :type (cl:vector rosky_msgs-msg:LEDDetection)
   :initform (cl:make-array 0 :element-type 'rosky_msgs-msg:LEDDetection :initial-element (cl:make-instance 'rosky_msgs-msg:LEDDetection))))
)
...
(cl:defmethod cl:initialize-instance :after ((m <LEDDetectionArray>) cl:&rest args)
  ...
  (cl:unless (cl:typep m 'LEDDetectionArray)
    (roslisp-msg-protocol:msg-deprecation-warning "using old message class name rosky_msgs-msg:<LEDDetectionArray> is deprecated: use rosky_msgs-msg:LEDDetectionArray instead.")))
...
(cl:defmethod roslisp-msg-protocol:deserialize ((msg <LEDDetectionArray>) istream)
  ...
  (cl:let ((vals (cl:slot-value msg 'detections)))
    ...
    (cl:setf (cl:aref vals i) (cl:make-instance 'rosky_msgs-msg:LEDDetection))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<LEDDetectionArray>)))
  ...
  (cl:format cl:nil "LEDDetection[] detections~%================================================================================~%MSG: rosky_msgs/LEDDetection~%time timestamp1		# initial timestamp of the camera stream used ~%time timestamp2		# final timestamp of the camera stream used ~%Vector2D pixels_normalized~%float32 frequency ~%string color        # will be ‘r’, ‘g’ or ‘b’ ~%float32 confidence  # some value of confidence for the detection (TBD)~%~%# for debug/visualization~%float64[] signal_ts~%float32[] signal~%float32[] fft_fs~%float32[] fft~%~%================================================================================~%MSG: rosky_msgs/Vector2D~%float32 x~%float32 y~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'LEDDetectionArray)))
  "Returns full string definition for message of type 'LEDDetectionArray"
  (cl:format cl:nil "LEDDetection[] detections~%================================================================================~%MSG: rosky_msgs/LEDDetection~%time timestamp1		# initial timestamp of the camera stream used ~%time timestamp2		# final timestamp of the camera stream used ~%Vector2D pixels_normalized~%float32 frequency ~%string color        # will be ‘r’, ‘g’ or ‘b’ ~%float32 confidence  # some value of confidence for the detection (TBD)~%~%# for debug/visualization~%float64[] signal_ts~%float32[] signal~%float32[] fft_fs~%float32[] fft~%~%================================================================================~%MSG: rosky_msgs/Vector2D~%float32 x~%float32 y~%~%~%"))
